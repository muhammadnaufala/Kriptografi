...
public class DesEncrypter {
    ...
    public List<String> Ktemp = new ArrayList<>();
    ...
    public String getLeftBiner(String plaintextBiner){
        return plaintextBiner.substring(0,32);
    }

    public String getRighBiner(String plaintextBiner){
    ...
    public String encrypt(String plaintext, String key) throws UnsupportedEncodingException {
        ...
        List<String> chipers = new ArrayList<>();
        ...
        for (String plaintextBlock : plaintextBlocks){ //tiap blok 64bit
            ...
            leftPlaintext = getLeftBiner(plaintextBlock);
            rightPlaintext = getRighBiner(plaintextBlock);
...
//                System.out.println("");
//            }
            chipers.add(permutation(L.get(16)+R.get(16),matrixIvIp));
            K.clear();
            C.clear();
            D.clear();
            L.clear();
    ...
    public String decrypt(String chipertext, String key) throws UnsupportedEncodingException{
        chipertext = teksToBiner(chipertext);
        List<String>chipertextBlocks = makeBlocks(chipertext,64); //mengecek tiap blok harus 64bit
        List<String> K = new ArrayList<>();
        List<String> C = new ArrayList<>();
        List<String> D = new ArrayList<>();
        List<String> L = new ArrayList<>();
        ...
        String plainFinal = "";
        String keyTemp = key;
        String leftChipertext = "";
        String rightChipertext = "";
        String k = "";
        String a = "";
        String b = "";
        initSBoxes();

        for (String chipertextBlock : chipertextBlocks){ //tiap blok 64bit
            PB.add("");
            B.add("");
            A.add("");
            chipertextBlock = permutation(chipertextBlock,matrixIp);
            leftChipertext = getLeftBiner(chipertextBlock);
            rightChipertext = getRighBiner(chipertextBlock);
            L.add(leftChipertext); //L0
            R.add(rightChipertext); //R0
            ...
            for (int n=0; n<16; n++){
                ...
                a = xor(Ktemp.get(15-n),expand(R.get(n),matrixExpand));
                A.add(a);
                b = sBox(A.get(n+1),this.sBoxes);
                B.add(b); //B1
                b = permutation(b,pBox);
                PB.add(b); //PB1
                leftChipertext = R.get(n);
                L.add(leftChipertext);
                rightChipertext = xor(L.get(n),PB.get(n+1));
                R.add(rightChipertext);
            }
...
//                System.out.println("");
//            }
            plains.add(permutation(L.get(16)+R.get(16),matrixIvIp));
            K.clear();
            C.clear();
            D.clear();
            L.clear();
